    ...
    */
    Data_Stream(int channel, double center_frequency, double sample_rate, Radio_Method method, double analog_lpf_bandwidth, double digital_lpf_bandwidth) : Channel(channel), Center_Frequency(center_frequency), Sample_Rate(sample_rate), Method(method)
    {
        ...
        try
        {
            ...
            if(LMS_SetSampleRate(__RADIO__.address,Sample_Rate,1) != 0) throw Error("Error setting sample rate "+std::to_string(sample_rate)+" HZ. (Is the sample rate in range?)");
            if(LMS_SetGaindB(__RADIO__.address,type,Channel,65) != 0) throw Error("Error setting gain to 0.7 of max. (Is the radio broken?)");
